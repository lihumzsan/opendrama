...
function readUploadName(request: NextRequest): string {
  ...
  if (!encodedName) throw new Error('VIDEO_TOOL_UPLOAD_REQUIRED')
  try {
...
function readUploadLength(request: NextRequest): number {
  ...
  if (!rawLength) throw new Error('VIDEO_TOOL_UPLOAD_LENGTH_REQUIRED')
  if (!/^\d+$/.test(rawLength)) throw new Error('VIDEO_TOOL_UPLOAD_LENGTH_INVALID')
  const contentLength = Number(rawLength)
  if (!Number.isSafeInteger(contentLength)) throw new Error('VIDEO_TOOL_UPLOAD_LENGTH_INVALID')
...
export const POST = apiHandler(async (request: NextRequest) => {
  ...
  if (!request.body) throw invalidUpload(new Error('VIDEO_TOOL_UPLOAD_REQUIRED'))
  const key = buildVideoToolInputKey(authResult.session.user.id, metadata.extension)
  try {
