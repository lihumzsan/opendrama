...
describe('MiniMax H3 prompt planner', () => {
  ...
  it('asks Codex for H3-ready timeline, camera, and audio-sync details without enabling dialogue', async () => {
    ...
    expect(prompt).toContain('Project knowledge-base preference guidance')
    expect(prompt).toContain('recommendations only')
    expect(prompt).toContain('one main motion')
    expect(prompt).toContain('timed beats')
    expect(prompt).toContain('From 0.00 to')
